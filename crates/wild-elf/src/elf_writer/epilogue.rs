use super::{
    ElfLayout, EpilogueOffsets, ProgramHeaderWriter, SymbolTableWriter, TableWriter,
    input_section_buffer_split, populate_file_header, script_sorted_buffer_split,
    write_dynamic_symbol_definitions, write_epilogue_dynamic_entries, write_gnu_hash_tables,
    write_got_plt_syms, write_internal_symbols, write_object_section, write_program_headers,
    write_section_headers, write_symbol_table_entries, write_sysv_hash_table, write_verdef,
};
use crate as elf;
use crate::writable_elf::WritableNoteHeader as _;
use crate::{
    ElfClass, ElfWord as _, GNU_NOTE_NAME, NoteProperty, RiscVAttribute, output_section_id, part_id,
};
use linker_utils::elf::riscvattr::{
    TAG_RISCV_ARCH, TAG_RISCV_PRIV_SPEC, TAG_RISCV_PRIV_SPEC_MINOR, TAG_RISCV_PRIV_SPEC_REVISION,
    TAG_RISCV_STACK_ALIGN, TAG_RISCV_UNALIGNED_ACCESS, TAG_RISCV_WHOLE_FILE,
};
use linker_utils::elf::{RISCV_ATTRIBUTE_VENDOR_NAME, secnames};
