...
const LIBC_INCR_DIRTY_TEST: &str = "elf/x86_64/glibc-libc-incremental-dirty";
const LIBM_INCR_TEST: &str = "elf/x86_64/glibc-libm-incremental";
const LIBM_INCR_DIRTY_TEST: &str = "elf/x86_64/glibc-libm-incremental-dirty";
...
fn run_libc_incremental_dirty_test() -> Result<libtest_mimic::Completion> {
    let Some((_tree, build)) = glibc_paths()? else {
        return Ok(libtest_mimic::Completion::ignored_with(format!(
            "{TREE_VAR} is unset"
        )));
    ...
    let Some(dirty) = abi_note.clone() else {
        return Ok(libtest_mimic::Completion::ignored_with(
            "glibc build has no csu/abi-note.o to dirty".to_owned(),
        ));
    };

    let out_dir = build_dir().join(LIBC_INCR_DIRTY_TEST);
    std::fs::create_dir_all(&out_dir)
        .with_context(|| format!("Failed to create {}", out_dir.display()))?;
    let out = out_dir.join("libc.so.incr.dirty.wild");
    let libgcc = libgcc_archive()?;
    let command = || {
        libc_command(
            &out,
...
fn run_libm_incremental_dirty_test() -> Result<libtest_mimic::Completion> {
    let Some((_tree, build)) = glibc_paths()? else {
        return Ok(libtest_mimic::Completion::ignored_with(format!(
            "{TREE_VAR} is unset"
        )));
    ...
    let Some(dirty) = first_existing(&build, &["csu/abi-note.o"]) else {
        return Ok(libtest_mimic::Completion::ignored_with(
            "glibc build has no csu/abi-note.o to dirty".to_owned(),
        ));
    };

    let out_dir = build_dir().join(LIBM_INCR_DIRTY_TEST);
    std::fs::create_dir_all(&out_dir)
        .with_context(|| format!("Failed to create {}", out_dir.display()))?;
    let out = out_dir.join("libm.so.incr.dirty.wild");
    let command = || -> Result<Command> { glibc_pic_command(spec, &out, &build, &pic, &libc) };
    incremental_check::relink_changed(
        with_incremental(command()?),
        with_incremental(command()?),
