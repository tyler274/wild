...
#[debug("sym-{_0}")]
pub(crate) struct SymbolId(pub(super) u32);

pub(super) struct AtomicSymbolId(AtomicU32);
...
impl SymbolId {
    ...
    pub(super) fn as_atomic(self) -> AtomicSymbolId {
        AtomicSymbolId(AtomicU32::new(self.0))
    }
}

impl AtomicSymbolId {
    pub(super) fn store(&self, selected: SymbolId) {
        self.0.store(selected.0, Ordering::Relaxed);
    }

    pub(super) fn into_non_atomic(self) -> SymbolId {
